...
struct File_Manager *Finder(struct File_Manager *Directory, char name[], int flag)
{
    ...
    else
    {
        ...
        {
            Dir = Finder(Directory->dirs[i], name, flag);
            if (Dir != NULL)
            {
                return Dir;
            }
...
void ls(struct File_Manager *Directory)
{
    ...
    // qsort(names, Directory->num, sizeof(char *), compare_strings);
...
struct File_Manager *cd(struct File_Manager *RootDir, char name[], struct File_Manager *CurrDir)
{
    int flag = 0;
    ...
    for (; Q > 0; Q--)
    {
        ...
        if (n == 2)
        {
            ...
            if (!(strcmp(op, "cd")))
            {
                ...
                {
                    CurrDir = cd(RootDir, name, CurrDir);
                }
            }
        }
        if (n == 3)
        {
            if (!(strcmp(op, "rm")))
            {
                ...
                if (!(strcmp(name, "-r")))
                {
                    scanf("%s", name);
                    int flag = 0;
                    struct File_Manager *Dir = Finder(RootDir, name, flag);
                    if (Dir == NULL)
                    {
                        printf("-1\n");
                    }
